...
- Ningún test sale a la red. Cuando vuelva a haber servicios externos, se
  mockean con fixtures **automáticas**: si hubiera que acordarse de pedirlas,
  el día que alguien escriba un test sin pedirla la suite sale a Internet.
...
async def _crear_usuario(
    db: AsyncSession, nombre: str, email: str, rol: RolUsuario
) -> Usuario:
...
async def usuario_admin(db: AsyncSession) -> Usuario:
    ...
    return await _crear_usuario(
        db, "Admin Prueba", "admin@prueba.com.ar", RolUsuario.admin
    )


@pytest_asyncio.fixture
async def usuario_encargado(db: AsyncSession) -> Usuario:
    """Usuario con rol encargado."""
    return await _crear_usuario(
        db, "Encargado Prueba", "encargado@prueba.com.ar", RolUsuario.encargado
    )


@pytest_asyncio.fixture
async def usuario_vendedor(db: AsyncSession) -> Usuario:
    """Usuario con rol vendedor."""
    return await _crear_usuario(
        db, "Vendedor Prueba", "vendedor@prueba.com.ar", RolUsuario.vendedor
    )
